...
public class EulaOrNewVersion
{
	...
	/**
	 * Extra for main intent. Specifies activity that should be launched after
	...
	static boolean showEula(Activity activity)
	{
		...
		{
			if (debug)
				Log.d(TAG, "Eula has been accepted.");
			return false;
		} else
		{
			if (debug)
				Log.d(TAG, "Eula has not been accepted yet.");
	...
	static boolean showNewVersion(Activity activity)
	{
		...
		{
			if (debug)
				Log.i(TAG, "Same version " + lastVersion + " as last launch.");
			return false;
		} else
		{
			if (debug)
				Log.i(TAG, "Newer version " + thisVersion
						+ " since last launch " + lastVersion
						+ ". Show recent changes.");
	...
	private static void startForwardActivity(Activity activity,
			Class launchClass)
	{
