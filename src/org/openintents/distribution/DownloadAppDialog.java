...
public class DownloadAppDialog extends AlertDialog implements OnClickListener
{
	...
	public DownloadAppDialog(Context context, int message_id,
			int download_name_id, int download_package_id,
			int download_website_id)
	{
		super(context);
		mContext = context;
		set(message_id, download_name_id, download_package_id,
				download_website_id);
	}

	public DownloadAppDialog(Context context, String message,
			String download_name, String download_package,
			String download_website)
	{
		super(context);
		mContext = context;
			...
			String download_package, String download_website)
	{
		...
		mMarketAvailable = MarketUtils.isMarketAvailable(mContext,
				mDownloadPackageName);
		...
		if (mMarketAvailable)
		{
			sb.append(mContext.getString(
					R.string.oi_distribution_download_market_message,
					mDownloadAppName));
		} else
		{
			sb.append(mContext
					.getString(R.string.oi_distribution_download_message,
							mDownloadAppName));
		}
		...
		setButton(mContext.getText(R.string.oi_distribution_download_market),
				this);
		setButton2(mContext.getText(R.string.oi_distribution_download_web),
				this);
	...
	public static void onPrepareDialog(Context context, Dialog dialog)
	{
		...
		boolean has_android_market = MarketUtils.isMarketAvailable(context,
				d.mDownloadPackageName);
	...
	public void startSaveActivity(Intent intent)
	{
		...
		{
			Toast.makeText(mContext, R.string.oi_distribution_update_error,
